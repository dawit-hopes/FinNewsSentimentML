...
class Visualizer:
    ...
    def plot_sma_ema_close(self):
        ...
        ax.plot(self.data.index, self.data['SMA'], label='SMA', color='orange', linestyle='--')
        ax.plot(self.data.index, self.data['EMA'], label='EMA', color='green', linestyle='-')
        ax.set_title('Closing Prices with SMA and EMA')
        ax.set_xlabel('Date')
        ax.set_ylabel('Price')
        ax.legend()
        plt.show()
